...
export default function Expenses() {
  ...
  const [form, setForm] = useState({ date: new Date().toISOString().slice(0, 10), account: '', paid_from: '', amount: '', payee: '', notes: '' })
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      await api.post('/api/accounting/expenses/', { ...form, amount: Number(form.amount) })
      toast.success(t('expense_added'))
      setShowForm(false)
      setForm({ date: new Date().toISOString().slice(0, 10), account: '', paid_from: '', amount: '', payee: '', notes: '' })
      fetchExpenses()
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowForm(true)} className="btn-primary"><Plus size={16} /> {t('add_expense')}</button>
        </div>
      </div>
      ...
      <div className="card p-0 overflow-x-auto">
        <table className="w-full text-sm min-w-[640px]">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>{[t('th_date'), t('exp_account'), t('exp_payee'), t('exp_paid_from'), t('th_amount')].map((h, i) => <th key={i} className="text-start px-4 py-3 text-xs font-semibold text-gray-500 uppercase">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {expenses.length === 0 ? (
              <tr><td colSpan={5} className="text-center py-12 text-gray-400"><Wallet size={40} className="mx-auto mb-2 opacity-30" />{t('no_expenses')}</td></tr>
            ) : expenses.map(x => (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between p-6 border-b">
              <h2 className="text-lg font-semibold">{t('add_expense')}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button type="submit" className="btn-primary flex-1 justify-center">{t('save_payment')}</button>
